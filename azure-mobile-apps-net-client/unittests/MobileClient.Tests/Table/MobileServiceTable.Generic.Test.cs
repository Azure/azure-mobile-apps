...
{
    public class MobileServiceTable_Generic_Test
    {
        [Theory]
        [InlineData("http://test.com/", "http://test.com/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com", "http://test.com/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com/folder/", "http://test.com/folder/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com/folder", "http://test.com/folder/about?$filter=a eq b&$orderby=c")]
        public async Task ReadAsync_WithAbsoluteUri_Generic(string serviceUri, string result)
        {
            ...
            await table.ReadAsync<ToDo>(result);

            Assert.Equal("TT,LH", hijack.Request.Headers.GetValues("X-ZUMO-FEATURES").First());
            Assert.Equal(result, hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task ReadAsync_Throws_IfAbsoluteUriHostNameDoesNotMatch()
        {
            ...
            IMobileServiceTable<ToDo> table = service.GetTable<ToDo>();
            await Assert.ThrowsAsync<ArgumentException>(() => table.ReadAsync<ToDo>("http://www.contoso.com/about?$filter=a eq b&$orderby=c"));
        }

        [Theory]
        [InlineData("http://test.com/", "/about?$filter=a eq b&$orderby=c", "http://test.com/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com", "/about?$filter=a eq b&$orderby=c", "http://test.com/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com/folder/", "/about?$filter=a eq b&$orderby=c", "http://test.com/folder/about?$filter=a eq b&$orderby=c")]
        [InlineData("http://test.com/folder", "/about?$filter=a eq b&$orderby=c", "http://test.com/folder/about?$filter=a eq b&$orderby=c")]
        public async Task ReadAsync_WithRelativeUri_Generic(string serviceUri, string queryUri, string requestUri)
        {
            var hijack = new TestHttpHandler();
            hijack.SetResponseContent("[{\"col1\":\"Hey\"}]");
            IMobileServiceClient service = new MobileServiceClient(serviceUri, hijack);
            ...
            await table.ReadAsync<ToDo>(queryUri);

            Assert.Equal("TT", hijack.Request.Headers.GetValues("X-ZUMO-FEATURES").First());
            Assert.Equal(requestUri, hijack.Request.RequestUri.ToString());
        }

        [Fact]
        public Task ReadAsync_Returns_IQueryResultEnumerable()
            => TestIQueryResultEnumerable(table => table.ReadAsync<ToDo>("this is a query"));

        [Fact]
        public Task ToCollectionAsync_Returns_IQueryResultEnumerable()
            => TestIQueryResultEnumerable(async table => await table.CreateQuery().ToCollectionAsync());

        [Fact]
        public Task ToEnumerableAsync_Returns_IQueryResultEnumerable()
            => TestIQueryResultEnumerable(async table => await table.CreateQuery().ToEnumerableAsync());

        [Fact]
        public Task ToListAsync_Returns_IQueryResultEnumerable()
            => TestIQueryResultEnumerable(async table => await table.CreateQuery().ToListAsync());
        ...
        private static async Task TestIQueryResultEnumerable(Func<IMobileServiceTable<ToDo>, Task<IEnumerable<ToDo>>> action)
        {
            ...
            Assert.Equal(23L, item.Id);
            Assert.Equal("Hey", item.Title);
            Assert.True(item.Complete);
            var provider = result as IQueryResultEnumerable<ToDo>;
            Assert.Equal(53L, provider.TotalCount);
            Assert.Equal("http://contoso.com/tables/Todo?$top=1&$skip=2", provider.NextLink);
        }
        ...
        [Fact] // this is the default buggy behavior that we've already shipped
        public async Task ReadAsync_ModifiesStringId_IfItContainsIsoDateValue()
        {
            ...
            Assert.Equal("01/29/2014 23:01:33", item.Id);
            Assert.Equal(DateTime.Parse("2014-01-29T23:01:33.444Z"), item.CreatedAt);
        }

        [Fact] // user has to set the serializer setting to round trip dates in string fields correctly
        public async Task ReadAsync_DoesNotModifyStringId_IfItContainsIsoDateValueAndSerializerIsConfiguredToNotParseDates()
        {
            ...
            Assert.Equal("2014-01-29T23:01:33.444Z", item.Id);
            Assert.Equal(DateTime.Parse("2014-01-29T23:01:33.444Z"), item.CreatedAt);
        }
        ...
        public async Task ReadAsyncWithStringIdTypeAndStringIdResponseContent()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Assert.Equal("Hey", items[0].String);
            }
        }
        ...
        public async Task ReadAsyncWithStringIdTypeAndNonStringIdResponseContent()
        {
            ...
            foreach (object testId in testIdData)
            {
                ...
                Assert.Single(items);
                Assert.Equal(testId.ToString(), items[0].Id);
                Assert.Equal("Hey", items[0].String);
            }
        }
        ...
        public async Task ReadAsyncWithStringIdTypeAndStringIdProjection()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Assert.Single(items);
                Assert.Equal(testId, items[0].Id);
                Assert.Equal("Hey", items[0].Message);
                Assert.Equal(hijack.Request.RequestUri.AbsoluteUri, expectedUri.AbsoluteUri);
            }
        }
        ...
        public async Task ReadAsyncWithIntIdTypeAndIntParseableIdResponseContent()
        {
            ...
            foreach (object testId in testIdData)
            {
                ...
                Assert.Single(items);
                Assert.Equal(Convert.ToInt64(testId), items[0].Id);
                Assert.Equal("Hey", items[0].String);
            }
        }
        ...
        public async Task ReadAsyncWithIntIdTypeAndStringIdResponseContent()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Assert.Contains("Error converting value", exception.Message);
            }
        }
...
#pragma warning disable CS0472 // The result of the expression is always the same since a value of this type is never equal to 'null'
            List<LongIdType> items = await table.Where(t => t.Id == null).ToListAsync().ConfigureAwait(false);
#pragma warning restore CS0472 // The result of the expression is always the same since a value of this type is never equal to 'null'
        ...
        public async Task LookupAsyncWithStringIdTypeAndNonStringIdResponseContent()
        {
            ...
            foreach (object testId in testIdData)
            {
                ...
                Assert.Equal(testId.ToString(), item.Id);
                Assert.Equal("Hey", item.String);
            }
        }
        ...
        public async Task LookupAsyncWithStringIdTypeAndInvalidStringIdParameter()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                IMobileServiceTable<StringIdType> table = service.GetTable<StringIdType>();
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(testId));
        ...
        public async Task LookupAsyncWithStringIdTypeAndIntIdParameter()
        {
            ...
            foreach (long testId in testIdData)
            {
                ...
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(testId));
                Assert.Contains("is invalid for looking up items of type", exception.Message);
            }
        }
        ...
        public async Task LookupAsyncWithStringIdTypeAndNullIdParameter()
        {
            ...
            Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(null));
        ...
        public async Task LookupAsyncWithIntIdTypeAndIntParseableIdResponseContent()
        {
            ...
            foreach (object testId in testIdData)
            {
                ...
                Assert.Equal(Convert.ToInt64(testId), item.Id);
                Assert.Equal("Hey", item.String);
            }
        }
        ...
        public async Task LookupAsyncWithIntIdTypeAndStringIdResponseContent()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(10));
                Assert.Contains("Error converting value", exception.Message);
            }
        }
        ...
        public async Task LookupAsyncWithIntIdTypeAndInvalidIntIdParameter()
        {
            ...
            foreach (long testId in testIdData)
            {
                ...
                IMobileServiceTable<LongIdType> table = service.GetTable<LongIdType>();
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(testId));
                Assert.True(exception.Message.Contains("Specified argument was out of the range of valid values") ||
        ...
        public async Task LookupAsyncWithIntIdTypeAndStringIdParameter()
        {
            ...
            Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync("a string"));
            Assert.Contains("is invalid for looking up items of type", exception.Message);
        }
        ...
        public async Task LookupAsyncWithIntIdTypeAndZeroIdParameter()
        {
            ...
            Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.LookupAsync(0L));
            Assert.Contains(" is not a positive integer value", exception.Message);
        }
        ...
        public async Task LookupAsyncWithUserParameters()
        {
            ...
            Assert.Contains("12", hijack.Request.RequestUri.ToString());
            Assert.Contains("state=CA", hijack.Request.RequestUri.Query);
            Assert.Equal(12, expected.Id);
            Assert.Equal("Hello", expected.String);
        }
        ...
        public async Task RefreshAsyncWithStringIdTypeAndInvalidStringIdParameter()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.RefreshAsync(item));
                Assert.True(exception.Message.Contains("An id must not contain any control characters or the characters") ||
                              exception.Message.Contains("is longer than the max string id length of 255 characters"));
            }
        }
        ...
        public async Task RefreshAsyncWithUserParameters()
        {
            ...
            Assert.Contains("state=CA", hijack.Request.RequestUri.Query);
            Assert.Equal(12, expected.Id);
            Assert.Equal("Goodbye", expected.String);
        }
        ...
        public async Task RefreshAsyncThrowsWhenNotFound()
        {
            ...
            await Assert.ThrowsAsync<MobileServiceInvalidOperationException>(() => table.RefreshAsync(new StringType() { Id = 5, String = "Just Created" }));
        }
        ...
        public async Task InsertAsync_DerivedTypeOnBaseTable_Succeeds()
        {
            ...
            Assert.Equal(23L, item.Id);
            Assert.Equal("Hey", item.PublicProperty);
        }
        ...
        public async Task InsertAsyncWithStringIdTypeAndStringIdItem()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                hijack.OnSendingRequest = async request =>
                {
                    ...
                    Assert.Equal(testId, (string)itemAsJObject["id"]);
                    Assert.Equal("what?", (string)itemAsJObject["String"]);
                    return request;
                };
        ...
        public async Task InsertAsyncWithStringIdTypeAndNullIdItem()
        {
            ...
            hijack.OnSendingRequest = async request =>
            {
                string requestContent = await request.Content.ReadAsStringAsync();
                JObject itemAsJObject = JObject.Parse(requestContent);
                Assert.Null((string)itemAsJObject["id"]);
                Assert.Equal("what?", (string)itemAsJObject["String"]);
                return request;
            };
        ...
        public async Task InsertAsyncWithStringIdTypeAndInvalidStringIdParameter()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.UpdateAsync(item));
                Assert.True(exception.Message.Contains("An id must not contain any control characters or the characters") ||
                              exception.Message.Contains("is longer than the max string id length of 255 characters"));
            }
        }
        ...
        public async Task InsertAsyncWithStringIdAndList_DoesNotDuplicateList()
        {
            ...
            Assert.Equal("goodbye", item.Values[0]);
            Assert.Equal("universe", item.Values[1]);
        }
        ...
        public async Task InsertAsyncWithIntIdTypeAndStringIdResponseContent()
        {
            ...
            Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.InsertAsync(item));
            Assert.Contains("Error converting value", exception.Message);
        }
        ...
        public async Task InsertAsyncWithIntIdTypeAndInvalidIntIdParameter()
        {
            ...
            foreach (long testId in testIdData)
            {
                ...
                Assert.True(exception.Message.Contains(" is not a positive integer value") ||
                              exception.Message.Contains("for member id is outside the valid range for numeric columns"));
            }
        }
        ...
        public async Task InsertAsyncGenericWithUserParameters()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("state=CA", hijack.Request.RequestUri.Query);
        }
        ...
        public async Task UpdateAsyncWithStringIdTypeAndStringIdItem()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                hijack.OnSendingRequest = async request =>
                {
                    ...
                    Assert.Equal(request.RequestUri.AbsoluteUri, expectedUri.AbsoluteUri);
                    return request;
                };
        ...
        public async Task DeleteAsyncWithStringIdTypeAndEmptyStringIdItem()
        {
            ...
            foreach (string testId in testIdData)
            {
                ...
                Exception exception = await Assert.ThrowsAnyAsync<Exception>(() => table.DeleteAsync(item));
                Assert.Contains("The id can not be null or an empty string.", exception.Message);
            }
        }
        ...
        public async Task DeleteAsyncWithUserParameters()
        {
            ...
            Assert.Contains("state=WY", hijack.Request.RequestUri.Query);
        }
        ...
        public async Task UndeleteAsync()
        {
            ...
            Assert.Equal("an id", obj.Id);
            Assert.Equal("Hey", obj.String);
        }
        ...
        public async Task IncludeDeleted()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("__includeDeleted=true", hijack.Request.RequestUri.ToString());

            Assert.Equal(12, people[0].Id);
            Assert.Equal("Hey", people[0].String);
        }
        ...
        public async Task IncludeTotalGenericList()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("$inlinecount=allpages", hijack.Request.RequestUri.ToString());

            Assert.Equal((long)1, people.TotalCount);
            Assert.Equal(12, people[0].Id);
            Assert.Equal("Hey", people[0].String);
        }
        ...
        public async Task IncludeTotalGenericEnum()
        {
            ...
            Assert.Equal((long)1, results.TotalCount);
            Assert.Equal(12, people[0].Id);
            Assert.Equal("Hey", people[0].String);
        }
        ...
        public async Task WhereAsyncWithStringIdGeneric()
        {
            ...
            Assert.Contains("StringIdType", hijack.Request.RequestUri.ToString());
            Assert.Contains("$filter=(id eq '12')", hijack.Request.RequestUri.ToString());
        ...
        public async Task OrderByAsyncGeneric()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("orderby=id,String", hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task OrderByDoubleAsyncGeneric()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("orderby=String,id", hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task OrderByDescAsyncGeneric()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("orderby=id desc,String desc", hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task OrderByAscDescAsyncGeneric()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("orderby=id,String desc", hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task SkipAndTakeAsyncGeneric()
        {
            ...
            Assert.Contains("StringType", hijack.Request.RequestUri.ToString());
            Assert.Contains("$skip=100", hijack.Request.RequestUri.ToString());
            Assert.Contains("$top=10", hijack.Request.RequestUri.ToString());
        }
        ...
        public async Task InsertAsyncStringIdSystemPropertiesRemovedFromRequest()
        {
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                Assert.False(obj.Properties().Where(p => p.Name.Contains("created")).Any());
                return request;
            };
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.False(obj.Properties().Where(p => p.Name.Contains("createdAt")).Any());
                return request;
            };
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.False(obj.Properties().Where(p => p.Name.Contains("updated")).Any());
                return request;
            };
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.False(obj.Properties().Where(p => p.Name.Contains("version")).Any());
                return request;
            };
        ...
        public async Task InsertAsyncStringIdNonSystemPropertiesNotRemovedFromRequest()
        {
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.True(obj.Properties().Where(p => p.Name.Contains("Created")).Any());
                return request;
            };
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.True(obj.Properties().Where(p => p.Name.Contains("Updated")).Any());
                return request;
            };
            ...
            hijack.OnSendingRequest = async (request) =>
            {
                ...
                JObject obj = JToken.Parse(content) as JObject;
                Assert.True(obj.Properties().Where(p => p.Name == "id").Any());
                Assert.True(obj.Properties().Where(p => p.Name.Contains("version")).Any());
                return request;
            };
        ...
        public async Task CreatedAtSystemPropertyDeserializesToDatetimeOrString()
        {
            ...
            Assert.Single(items);
            Assert.Equal("an id", items[0].Id);
            Assert.Equal(new DateTime(1999, 12, 31, 23, 59, 59, DateTimeKind.Utc).ToLocalTime(), items[0].CreatedAt);
            ...
            Assert.Single(stringItems);
            Assert.Equal("an id", stringItems[0].Id);

            // TODO: culture-specific, may fail in other formats
            Assert.Equal("12/31/1999 23:59:59", stringItems[0].CreatedAt);
        }
        ...
        public async Task UpdatedAtSystemPropertyDeserializesToDatetimeOrString()
        {
            ...
            Assert.Single(items);
            Assert.Equal("an id", items[0].Id);
            Assert.Equal(new DateTime(1999, 12, 31, 23, 59, 59, DateTimeKind.Utc).ToLocalTime(), items[0].UpdatedAt);
            ...
            // TODO: culture-specific, may fail in other formats
            Assert.Equal("12/31/1999 23:59:59", stringItems[0].UpdatedAt);
        }
        ...
        public async Task VersionSystemPropertyDeserializesToString()
        {
            ...
            Assert.Single(items);
            Assert.Equal("an id", items[0].Id);
            Assert.Equal("AAAAAAAAH2o=", items[0].Version);
        }

        [Fact]
        public Task UpdateAsync_SetsIfMatchHeader_WhenObjectHasVersionOnIt()
            => TestIfMatchHeaderIsSet((client, item) => client.UpdateAsync(item));

        [Fact]
        public Task DeleteAsync_SetsIfMatchHeader_WhenObjectHasVersionOnIt()
            => TestIfMatchHeaderIsSet((client, item) => client.DeleteAsync(item));
        ...
        private static async Task TestIfMatchHeaderIsSet(Func<IMobileServiceTable<VersionType>, VersionType, Task> action)
        {
            ...
            foreach (Tuple<string, string> testcase in testCases)
            {
                ...
                hijack.OnSendingRequest = (request) =>
                {
                    Assert.Equal(request.Headers.IfMatch.First().Tag, testcase.Item2);
        ...
        public async Task VersionSystemPropertySetFromEtagHeader()
        {
            ...
            foreach (Tuple<string, string> testcase in testCases)
            {
                ...
                Assert.Equal(item.Version, testcase.Item1);
            }
        }
