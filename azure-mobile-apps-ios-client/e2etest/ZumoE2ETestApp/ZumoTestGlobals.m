...
NSString *const CLIENT_VERSION_TAG = @"client-version-tag";
...
NSString *const TABLES_ROUND_TRIP_STRING_ID = @"RoundTripTable";
NSString *const TABLES_ROUND_TRIP_INT_ID = @"IntIdRoundTripTable";
...
+(BOOL)compareJson:(id)json1 with:(id)json2 log:(NSMutableArray *)errors {
    ...
    if (firstIsArray && secondIsArray) {
        ...
        if ([arr1 count] == [arr2 count]) {
            ...
            [errors addObject:[NSString stringWithFormat:@"Different array sizes - json1.len = %lu, json2.len = %lu",
                               (unsigned long)[arr1 count],
                               (unsigned long)[arr2 count]]];
            return NO;
        }
    ...
    if (firstIsObject && secondIsObject) {
        ...
        if ([dic1 count] == [dic2 count]) {
            ...
            [errors addObject:[NSString stringWithFormat:@"Different dictionary sizes - json1.len = %lu, json2.len = %lu",
                               (unsigned long)[dic1 count],
                               (unsigned long)[dic2 count]]];
            return NO;
        }
