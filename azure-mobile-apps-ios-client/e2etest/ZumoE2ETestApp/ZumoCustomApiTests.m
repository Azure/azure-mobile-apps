        ...
        [client invokeAPI:apiName body:nil HTTPMethod:@"GET" parameters:@{@"x":@"1"} headers:nil completion:^(id result, NSURLResponse *response, NSError *error) {
            if (shouldSucceed) {
                ...
                if ([httpResponse statusCode] == 401) {
                    ...
                    [test addLog:[NSString stringWithFormat:@"Error, expected 401, received %ld", (long)[httpResponse statusCode]]];
                    completion(NO);
                }
            }
        }];
    ...
    if (statusCode >= 400) {
        ...
        if (error) {
            ...
            if (statusCode == actualStatusCode) {
                ...
                [test addLog:[NSString stringWithFormat:@"Received incorrect status code. Expected %ld, received %ld", (long)statusCode, (long)actualStatusCode]];
                testPassed = NO;
            }
        } else {
        ...
        if (error) {
            ...
            [test addLog:[NSString stringWithFormat:@"Response status code: %ld; header: %@", (long)errorResponse.statusCode, errorResponse.allHeaderFields]];
            testPassed = NO;
        }
    }
