...
public class AzureSqlEntityTableRepository_Tests : RepositoryTests<EntityMovie>
{
    ...
    {
        connectionString = Environment.GetEnvironmentVariable("ZUMO_AZSQL_CONNECTIONSTRING");
        if (!string.IsNullOrEmpty(connectionString))
        {
            _context = new Lazy<AzureSqlDbContext>(() => AzureSqlDbContext.CreateContext(connectionString, output));
            movies = Context.Movies.AsNoTracking().ToList();
        }
    }

    private AzureSqlDbContext Context { get => _context.Value; }
