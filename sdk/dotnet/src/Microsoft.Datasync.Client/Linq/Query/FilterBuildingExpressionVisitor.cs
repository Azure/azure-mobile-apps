...
#pragma warning restore IDE0066 // Convert switch statement to expression
        ...
        private Expression VisitMethodCallExpression(MethodCallExpression node)
        {
            ...
            if (MethodNames.TryGetValue(key, out string methodName, out bool isStatic))
            {
                var fnCallNode = new FunctionCallNode(methodName);
                FilterExpression.Push(fnCallNode);
                foreach (var argument in (isStatic ? new Expression[0] : new Expression[] { node.Object }).Concat(node.Arguments))
                {
                    Visit(argument);
                }
                SetChildren(fnCallNode);
        ...
        internal bool IsEnumExpression(UnaryExpression expression) => expression.NodeType == ExpressionType.Convert && expression.Operand.Type.GetTypeInfo().IsEnum;
