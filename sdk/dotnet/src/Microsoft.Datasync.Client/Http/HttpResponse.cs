...
namespace Microsoft.Datasync.Client.Http
{
    ...
    {
        protected HttpResponse(HttpResponseMessage response)
        {
        ...
        internal static async Task<HttpResponse> FromResponseAsync(HttpResponseMessage message, CancellationToken token = default)
        {
            ...
            var response = new HttpResponse(message)
            {
        ...
        internal static Task<HttpResponse<T>> FromResponseAsync<T>(HttpResponseMessage message, JsonSerializerOptions deserializerOptions, CancellationToken token = default)
            => HttpResponse<T>.FromResponseAsync(message, deserializerOptions, token);
    ...
    /// <summary>
    /// A typed version of the <see cref="HttpResponse"/> object that deserializes expected JSON content
    ...
    public class HttpResponse<T> : HttpResponse
    {
        ...
        /// </summary>
        /// <param name="message">The source message</param>
        protected HttpResponse(HttpResponseMessage message) : base(message)
        {
        }
        ...
        /// <param name="token">A <see cref="CancellationToken"/></param>
        /// <returns>The <see cref="HttpMessage{T}"/></returns>
        internal static async Task<HttpResponse<T>> FromResponseAsync(HttpResponseMessage message, JsonSerializerOptions deserializerOptions, CancellationToken token = default)
        {
            Validate.IsNotNull(message, nameof(message));
            Validate.IsNotNull(deserializerOptions, nameof(deserializerOptions));

            var response = new HttpResponse<T>(message)
            {
                Content = await message.Content.ReadAsByteArrayAsync(token).ConfigureAwait(false)
            };
