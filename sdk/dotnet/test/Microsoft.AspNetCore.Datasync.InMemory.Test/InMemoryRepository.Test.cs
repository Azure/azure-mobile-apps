...
namespace Microsoft.AspNetCore.Datasync.InMemory.Test
{
    ...
    public class InMemoryRepository_Tests
    {
        ...
        public async Task Throws_TestException([CombinatorialRange(0, 5)] int op)
        {
            ...
            var repository = new InMemoryRepository<InMemoryMovie>
            {
                ThrowException = new DatasyncException("test exception")
            };
            ...
            // Act
            DatasyncException ex = null;
            switch (op)
            {
                case 0:
                    ex = await Assert.ThrowsAsync<DatasyncException>(() => repository.CreateAsync(item));
                    break;
                case 1:
                    ex = await Assert.ThrowsAsync<DatasyncException>(() => repository.DeleteAsync(item.Id));
                    break;
                case 2:
                    ex = await Assert.ThrowsAsync<DatasyncException>(() => repository.ReadAsync(item.Id));
                    break;
                case 3:
                    ex = await Assert.ThrowsAsync<DatasyncException>(() => repository.ReplaceAsync(item));
                    break;
                case 4:
                    ex = Assert.Throws<DatasyncException>(() => repository.AsQueryable());
                    break;
                default:
