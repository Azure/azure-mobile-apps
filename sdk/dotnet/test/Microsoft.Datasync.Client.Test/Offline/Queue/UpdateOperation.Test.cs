...
{
    private readonly JObject serializedObject = JObject.Parse("{\"kind\":3,\"itemId\":\"1234\",\"item\":null, \"sequence\":0, \"state\":0, \"tableName\":\"test\", \"version\":1}");
    ...
    {
        var sut = new UpdateOperation("test", "1234") { Item = testObject };
        var other = new UpdateOperation("test", "1234") { Item = testObject };
    ...
    public void Collapse_DeleteOperation()
    {
        ...
        Assert.False(sut.IsUpdated);
        Assert.True(newOp.IsUpdated);
    }
    ...
    public async Task ExecuteOnOfflineStore_Works_WithItemInStore()
    {
        ...
        await store.GetItemAsync("test", "1234", default);
        await store.UpsertAsync(default, Arg.Is<IEnumerable<JObject>>(ops => ops.Single().Equals(testObject)), default, default);
    }
