...
namespace Azure.Mobile.Test.E2EServer.Database
{
    ...
    {
        public static void Initialize(TableServiceContext context)
        {
            ...
            {
                SeedMoviesTable(context);
            }
        }

        public static void SeedMoviesTable(TableServiceContext context)
        {
            foreach (var movie in TestMovies)
            {
                // Fill in the basics for the strategy
                movie.Id = Guid.NewGuid().ToString("N");
                movie.Deleted = false;
                movie.Version = Guid.NewGuid().ToByteArray();
                movie.UpdatedAt = DateTimeOffset.UtcNow.AddDays(-(180 + (new Random()).Next(180)));
                context.Add(movie);
            }
            context.SaveChanges();
        }
